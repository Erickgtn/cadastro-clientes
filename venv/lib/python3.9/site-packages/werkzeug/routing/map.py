...
class Map:
    ...
    `Map` routes since those affect all rules, others are just defaults
...
class MapAdapter:
    ...
    def match(
        ...
        - you receive a `RequestRedirect` exception with a `new_url`
          ...
          You can use the `RequestRedirect` routes as response-like object
    ...
    def build(
        ...
        charset defined on the map routes.
        ...
        # Always build WebSocket resource with the scheme (browsers
        # require full URLs). If bound to a WebSocket, ensure that HTTP
        # resource are built with an HTTP scheme.
