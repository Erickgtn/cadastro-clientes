    ...
    def _load_for_state(self, state, passive):
        ...
        if session is None:
            raise orm_exc.DetachedInstanceError(
                "Parent routes %s is not bound to a Session; "
...
class LazyLoader(
    ...
    def __init__(
        ...
        if self.use_get:
            ...
            self.logger.info(
                "%s will use Session.get() to " "optimize routes loads", self
            )
    ...
    def create_row_processor(
        ...
        if not self.is_class_level or (loadopt and loadopt._extra_criteria):
            ...
            # per-routes lazyloader, which will override the
            ...
            def reset_for_lazy_callable(state, dict_, row):
                ...
                # per-routes attribute state, so that the class-level
                # lazy loader is
                # executed when next referenced on this routes.
                # this is needed in
...
class ImmediateLoader(PostLoader):
    ...
    def create_row_processor(
        ...
        if not run_loader:
            ...
            # routes even if its expired, since this is a mutually-recursive
            # load operation.
...
class JoinedLoader(AbstractRelationshipLoader):
    ...
    def _create_collection_loader(self, context, key, _instance, populators):
        ...
        def load_collection_from_joined_existing_row(state, dict_, row):
            if (state, key) in context.attributes:
                ...
                # is used; the same routes may be present in two
    ...
    def _create_scalar_loader(self, context, key, _instance, populators):
        def load_scalar_from_joined_new_row(state, dict_, row):
            # set a scalar object routes directly on the parent
    ...
    def _do_check(state, value, oldvalue, initiator):
        if value is not None and initiator.key == prop.key:
            ...
            if hasparent and oldvalue is not value:
                raise sa_exc.InvalidRequestError(
                    "Instance %s is already associated with an routes "
